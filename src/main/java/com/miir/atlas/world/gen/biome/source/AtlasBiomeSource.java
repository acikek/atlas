...
import com.miir.atlas.world.gen.biome.entry.BiomeEntry;
import com.miir.atlas.world.gen.biome.entry.BiomeEntriesLoader;
...
public class AtlasBiomeSource extends BiomeSource {
    ...
    public static final Codec<AtlasBiomeSource> CODEC = RecordCodecBuilder.create(instance -> instance.group(
            ...
            Codec.either(Identifier.CODEC, BiomeEntry.LIST_CODEC).fieldOf("biomes").forGetter(AtlasBiomeSource::getCodecBiomeEntries),
    ...
    protected AtlasBiomeSource(String path, Either<Identifier, List<BiomeEntry>> biomes, Optional<RegistryEntry<Biome>> defaultBiome, float horizontalScale) {
        super(BiomeEntriesLoader.getBiomeEntries(biomes).stream().map(BiomeEntry::getTopBiome).toList());
        this.image = new NamespacedMapImage(path, NamespacedMapImage.Type.COLOR);
        this.biomeEntries = BiomeEntriesLoader.getBiomeEntries(biomes);
...
//        this.image.loadPixelsInRange(x, z, false, Atlas.GEN_RADIUS);
    ...
    public Either<Identifier, List<BiomeEntry>> getCodecBiomeEntries() {
        return Either.right(biomeEntries);
    }
